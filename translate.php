...
//	echo "<b>found  term</b><br><pre>";
	foreach ($tags as $t ){
		$tag_params = preg_split("/\s+/",$t[3]);
		$lang="";
		$nodiv="";
		$para="";
		foreach ($tag_params as $a){
			$a = split("=",$a);
			if ($a[0] == "lang") $lang=$a[1];
			if ($a[0] == "para") $para=$a[1];
			if ($a[0] =="nodiv") $nodiv=$a[1];
		}
		if (array_key_exists($para,$paragraphs)){
			$storepara = $paragraphs[$para]; // have para
			if (array_key_exists($lang, $storepara) ){
				$storelang = $storepara[$lang]; // have lang
			} else {
				$storelang = $t[5];
				$storepara[$lang]= $storelang;
				$paragraphs[$para]= $storepara;
			}
		} else {
			$storepara[$lang]=$t[5];
			$paragraphs[$para]=$storepara;
	...
	$para = $atts['para']; // reset to current 

	$set_deflang = $para != "" && 
					array_key_exists($para, $paragraphs) && 
					array_key_exists($deflang, $paragraphs[$para]);

	$set_sesslang= $para != "" &&
					array_key_exists($para, $paragraphs) && 
					array_key_exists($sesslang, $paragraphs[$para]);
/*
	echo "...$para... : is done in $deflang ? - $set_deflang \n";
	echo "...$para... : is done in $sesslang ? - $set_sesslang \n";
...
*/
	...
	if ($paradone != $para ){
		if ($set_sesslang) {
			$trans =  $paragraphs[$para][$sesslang];
		} 
		else {
			$trans = $paragraphs[$para][$deflang];
		}
		$paradone = $para;

		if ( ! isset($nodiv)){
	    	$trans = "<div class='translate_".$lang."'>".$content."</div>";
		}
		return $trans;
	}
...
//	/*
//	echo "<pre>";
//	echo "lang in sc: $lang <br>";
//	echo "content in sc: $content <br>";
//	echo "</pre>";
//	 */
//
//
//	if ( $lang != $_SESSION['language'] && $content == null ) {
//
//		// find [translate] [/translate] in default language
//	
//		global $post;
//		$pcontent	= $post->post_content ; 
//		$pattern	= get_shortcode_regex();
//		$ret = preg_replace_callback('/'.$pattern.'/s', 
//
//			function ($matches ) use ($content){
//
//				global $wpdb, $tbllangs;
//
//				$deflang = $wpdb->get_var(
//					"SELECT name FROM $tbllangs WHERE main = 1");
//
//				$sc		= $matches[2];
//				$scc	= trim( $matches[5] );
//				$content= trim( $content );
//				$scargs	= preg_split("/\s+/", $matches[3]);
//
//				if ($sc== "translate" && $scc == $content ){
//
//
//					foreach ($scargs as $arg ){
//						$argval = split("=", $arg);
//
//						if ( $argval[0] == "lang"){
//							if ($argval[1] == $deflang){
...
//
//		$content = $ret;
//			echo "sess lang not tag lang<br>";
//	} 
//
//	/*
//	if ( ! isset($atts['nodiv'])){
...
function translate_title_quiet() {
